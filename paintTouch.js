...
window.onload = function () {
	...
	//attach the events needed to start drawing with the mouse
	canvas.onmousedown = startMouseDrawing;
	canvas.onmouseup = stopMouseDrawing;
	canvas.onmouseout = stopMouseDrawing;
	canvas.onmousemove = mouseDraw;
	
	//attach events needed to start drawing using a touchscreen
	//canvas.ontouchstart = canvasStartTouch;
	//canvas.ontouchmove = canvasMoveTouch;
	...
	if (!e)
	{
	 ...
	 if (e.touches)
	 {
		 if (e.touches.length == 1)	//only deal with one finger
		 {
			 ...
			 touchX = touch.pageX - touch.target.offsetLeft;	//get the x-coordinate of the touch on the canvas
			 touchY = touch.pageY - touch.target.offsetTop;		//get the y-coordinate of the touch on the canvas
